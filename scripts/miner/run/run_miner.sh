...
MODEL_PATH="${MODEL_PATH:-${POKER44_MODEL_PATH:-models/poker44_start_model_2.joblib}}"
...
echo "Config: netuid=$NETUID network=$NETWORK wallet=$WALLET_NAME hotkey=$HOTKEY axon_port=$AXON_PORT model_path=$MODEL_PATH"
if [ -n "$ALLOWED_VALIDATOR_HOTKEYS" ]; then
